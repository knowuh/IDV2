...
var width = document.getElementById('clusterSN').clientWidth-margin.l-margin.r,
  height = document.getElementById('clusterSN').clientHeight-margin.t-margin.b;



var plot = d3.select('.clusterSN')
  .append('svg')
...
var plotTW = d3.select('.clusterTW')
  .append('svg')
  .attr('width',width+margin.l+margin.r)
  .attr('height',height+margin.t+margin.b)
  .append('g')
...
var nodesEnter= nodes.enter()
    ...
    .attr('y',function(d){return scaleYhue(d.hue)})
    .attr('width',50)
    .attr("height", 50)
...
nodes.transition()
  force.nodes(data)
    ...
    d3.select('#brightSN').on('click',function(){
      force.stop()
        .on('tick',brightForce)
        .start()
    ...
    d3.select('#hueSN').on('click',function(){
      force.stop()
        .on('tick',hueForce)
        .start()
    ...
    d3.select('#satSN').on('click',function(){
      force.stop()
        .on('tick',satForce)
        .start()
...
var nodesTW = plotTW.selectAll ('.nodeTW')
  .data(data, function(d){return d.label});

var nodesEnter= nodesTW.enter()
    .append("image")
    .attr("class", "nodeTW")
    .attr("xlink:href", function(d){return 'starryNight/images/'+ d.label})
    .attr('x',function(d){return d.x})
...
nodes.transition()
  force.nodes(data)
    ...
    d3.select('#brightTW').on('click',function(){
      force.stop()
        .on('tick',brightForceTW)
        .start()

    });

    d3.select('#hueTW').on('click',function(){
      force.stop()
        .on('tick',hueForceTW)
        .start()
    ...
    d3.select('#satTW').on('click',function(){
      force.stop()
        .on('tick',satForceTW)
        .start()
